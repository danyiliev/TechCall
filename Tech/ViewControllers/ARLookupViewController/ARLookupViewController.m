...
- (void)viewDidAppear:(BOOL)animated {
    ...
    NSString        *column = [prefs stringForKey:@"SearchColumn"];
    if ([column isEqualToString:@"customer_name"])
    {
        [headerCell.m_comboField updateWithSelectedIndex:0];
    }
    else if ([column isEqualToString:@"cust_address1"])
    {
        [headerCell.m_comboField updateWithSelectedIndex:1];
    }
    
    else if ([column isEqualToString:@"customer_phone"])
    {
        [headerCell.m_comboField updateWithSelectedIndex:2];
    }
}
...
-(void)searchBtnClicked:(UIButton*)sender
{
    if (sender.tag == 0)
    {
        ...
        [manager GET:replacedURL parameters:nil success:^(AFHTTPRequestOperation * operation, id responseObject) {
            ...
            [CommonUtils saveSearchColumn:fields[headerCell.m_comboField.getSelectedIndex]];
