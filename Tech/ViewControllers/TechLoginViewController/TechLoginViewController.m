...
- (IBAction)onLogin:(id)sender {
    ...
    if (self.m_password.text.length < 1)
    {
        UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"Alert" message:@"Please input Password" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    ...
    if (self.m_optionTech.selectedSegmentIndex == 0)
        stCall = [NSString stringWithFormat:@"https://%@/api/TechCredential/IsTechnicianValid/%@?pPassword=%@", ipAddr, strspace, self.m_password.text];
    else
        stCall = [NSString stringWithFormat:@"https://%@/api/TechCredential/IsSalesmanValidNew/%@?pPassword=%@", ipAddr, strspace, self.m_password.text];
        //stCall = [NSString stringWithFormat:@"http://%@/api/TechCredential/IsSalesmanValidNew/%@?pPassword=%@", @"192.168.0.170:80", strspace, self.m_password.text];
    
    [manager GET:stCall parameters:nil success:^(AFHTTPRequestOperation * operation, id responseObject) {

        [SVProgressHUD dismiss];
        [CommonUtils saveLoginInfo:[responseObject objectForKey:@"AllowToCreateUnapplied"] :[responseObject objectForKey:@"AllowToPreAuthorize"] :[responseObject objectForKey:@"ShowArSearch"] :[responseObject objectForKey:@"isValid"] :[responseObject objectForKey:@"ErrorMessage"]];
        
        if (![responseObject objectForKey:@"isValid"])
        {
            UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"Invalid User" message:@"Invalid User" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            
            [alert show];
            return;
        }
        
        if ( [[responseObject objectForKey:@"ErrorMessage"] length] < 1)
        {
            if (self.m_optionTech.selectedSegmentIndex == 0)
        ...
        {
            UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"Technician/Salesman" message:@"Id or password does not match" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
