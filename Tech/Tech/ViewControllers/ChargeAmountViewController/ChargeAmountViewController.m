...
- (void)viewDidLoad {
    ...
    if ([techType isEqualToString:@"T"])
        savedCredit = [NSString stringWithFormat:@"https://%@/api/TechCredential/IsTechnicianValid/%@?pPassword=%@", ipAddr, loginTech, passwd];
    else if ([techType isEqualToString:@"S"])
        savedCredit = [NSString stringWithFormat:@"https://%@/api/TechCredential/IsSalesmanValidNew/%@?pPassword=%@", ipAddr, loginTech, passwd];
    
    [manager GET:savedCredit parameters:nil success:^(AFHTTPRequestOperation * operation, id responseObject) {
        
        [SVProgressHUD dismiss];
        NSDictionary *response = responseObject;
        BOOL preAuth = [response objectForKey:@"AllowToPreAuthorize"];
        
        if (!preAuth)
        {
            [self.m_preAuthSwitch setOn:NO];
            self.m_txtComments.hidden = YES;
            self.m_commentsLabel.hidden = YES;
        }
        else
        {
            [self.m_preAuthSwitch setOn:YES];
            self.m_txtComments.hidden = NO;
            self.m_commentsLabel.hidden = NO;
        }
...
//    self.m_preAuthSwitch.enabled = NO;
    ...
    [self.m_scrollView setContentSize:CGSizeMake([UIScreen mainScreen].bounds.size.width, 780)];
...
- (void)ChargeCard {
    ...
    else if (selectedSegmentIndex == 1)
    {
        ...
        //Main info
        Values = [NSArray arrayWithObjects:CreditCardDict, self.m_txtName.text, @"", [self.serviceCalls objectForKey:@"billToName"], amountCharge, pBillTo, @"", self.m_txtEmail.text, self.m_txtComments.text, preStr, nil];
        Keys = [NSArray arrayWithObjects:@"CreditCardInfo", @"ARName", @"InvoiceNumber", @"SMName", @"AmountToCharge", @"ARNum", @"CompanyCode", @"Comments", @"Email", @"PreAuthorizeOnly", nil];
